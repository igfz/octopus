//if code is changed,reload application cache
!function(b){var a=b.applicationCache;a.addEventListener("updateready",function(b){a.status==a.UPDATEREADY&&(a.swapCache(),location.reload())})}(window);
...
//模板引擎插件来自frozenui
!function(c){var b;b={};c.tpl=function(a,d,c){var e=/[^\w\-\.:]/.test(a)?function(a,c){var b,d=[],f=[];for(b in a)d.push(b),f.push(a[b]);return(new Function(d,e.code)).apply(c||a,f)}:b[a]=b[a]||this.get(document.getElementById(a).innerHTML);e.code=e.code||"var $parts=[]; $parts.push('"+a.replace(/\\/g,"\\\\").replace(/[\r\t\n]/g," ").split("<%").join("\t").replace(/(^|%>)[^\t]*/g,function(a){return a.replace(/'/g,"\\'")}).replace(/\t=(.*?)%>/g,"',$1,'").split("\t").join("');").split("%>").join("$parts.push('")+"'); return $parts.join('');";return d?e(d,c):e}}(window.Zepto);
...
//全局变量
var ajaxUrl = '//www.8win.com/jsky/phone/index.do',
		loadImg = '<li class="loading"><i></i></li>',
		SS = window.sessionStorage;
		ajaxParam = {'busiCode':'','src':'05xx00010100','product':'Micro site',
		'mobileInfo':{'platform':'h5','imei':'00000000000'},
		'userInfo':{'uid':'800214325435','jsessionId':'3213214214'},'commandInfo':{}};